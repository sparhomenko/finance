...
class Loader:
    ...
    def load(self):
        ...
        for file in sorted(listdir(self.path)):
            ...
            for element in one(extract_pages(f"{self.path}/{file}", laparams=LAParams(line_margin=-1))):
            ...
            bonus = table.get("Annual Booking Bonus", {}).get("Spec.Payment", 0)
            ...
            booking = table.get("Booking Benefit (Nett)", {}).get("Payment", 0)
